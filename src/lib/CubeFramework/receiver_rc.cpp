...
bool ElrsCfRcPublisher::publish(bool available, bool modelMatch, bool inhibited, const uint32_t *channels, uint32_t nowUs)
{
    ...
    for (unsigned i = 0; i < 16; i++)
        if (channels[i] > 2047 && channels[i] != CHANNEL_UNSET)
        {
            pending_ = false;
            return false;
        }
    ...
    for (unsigned i = 0; i < 16; i++)
        frame_.channels[i] = channels[i] == CHANNEL_UNSET ? CHANNEL_EMPTY : channels[i];
    stagedUs_ = nowUs;
    pending_ = true;
    return true;
}
...
void ElrsCfRcPublisher::sent(uint32_t sequence)
{
    ...
    if (pending_ && frame_.sequence == sequence)
        pending_ = false;
}
