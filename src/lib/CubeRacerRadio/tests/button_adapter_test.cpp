...
static void pinMode(uint8_t, int)
{
    modeWrites++;
}

static int digitalRead(uint8_t)
{
    return high;
}
...
static void step(Button &button, bool level, unsigned ms = 25)
{
    high = level;
    now += ms;
    button.update();
...
{
    Button button;
    button.OnShortPress = []() { shortPresses++; };
    button.OnLongPress = []() { longPresses++; };
    button.init(81);
    assert(modeWrites == 0);
    step(button, false, 600);
    step(button, false, 600);
    step(button, false, 600);
    ...
    step(button, true);
    step(button, true);
    assert(shortPresses == 1);
    step(button, false);
    step(button, false);
    step(button, false, 501);
    assert(longPresses == 1);
    owned = false;
    step(button, false, 600);
    step(button, true);
    assert(longPresses == 1 && shortPresses == 1);
    owned = true;
    step(button, true);
    step(button, true);
    assert(shortPresses == 1); // No stale release.
