...
int main()
{
    assert(LED_device.initialize());
    LED_device.start();
    LED_device.timeout();
    assert(genericWrites == 0 && writes > 0);
    assert(LED_device.initialize());
    assert(!high);
    assert(LED_device.event() == 500 && high);
    assert(LED_device.timeout() == 500 && !high);
    connectionState = connected;
    LED_device.event();
    assert(high);
    InBindingMode = true;
    assert(LED_device.event() == 100 && high);
    assert(LED_device.timeout() == 100 && !high);
    InBindingMode = false;
    connectionState = radioFailed;
    assert(LED_device.event() == 200 && high);
    assert(LED_device.timeout() == 1000 && !high);
