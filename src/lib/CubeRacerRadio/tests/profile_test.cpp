...
int main() {
    ...
    assert(hardware_pin(HARDWARE_radio_nss)==4);
    assert(hardware_pin(HARDWARE_radio_busy)==67);
    assert(hardware_pin(HARDWARE_radio_dio1)==20);
    ...
    for(auto pin:{HARDWARE_serial_rx,HARDWARE_serial_tx,HARDWARE_serial1_rx,HARDWARE_serial1_tx,
        ...
        HARDWARE_led_rgb,HARDWARE_i2c_sda,HARDWARE_vtx_nss,HARDWARE_misc_fan_pwm})assert(hardware_pin(pin)==-1);
