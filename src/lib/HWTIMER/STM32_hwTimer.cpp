...
#ifdef CUBERACER_M4
bool hwTimer::initialized() { return alreadyInit && callbackTick && callbackTock && cuberacerRadioTimerReady(); }
#endif
...
void hwTimer::stop()
{
    running = false;
#ifdef CUBERACER_M4
    if (!alreadyInit || !cuberacerTimerClockAllowed(reinterpret_cast<uintptr_t>(TIM1))) return;
#endif
    MyTim->pause();
    MyTim->setCount(0);
}
...
{
#ifdef CUBERACER_M4
    if (!alreadyInit || !cuberacerRadioTimerReady()) { stop(); return; }
#endif
...
{
#ifdef CUBERACER_M4
    if (!cuberacerRadioTimerReady()) { stop(); return; }
#endif
    if (hwTimer::isTick)
    {
#if defined(TARGET_TX)
