...
#define GPIO_PIN_NSS 20
#define LOW 0
#define HIGH 1
struct SPI_TypeDef { uint32_t SR; } peripheral;
enum { HAL_SPI_STATE_RESET, HAL_SPI_STATE_READY };
struct SPI_HandleTypeDef { unsigned State=HAL_SPI_STATE_READY; };
struct spi_t { SPI_TypeDef *spi=&peripheral; uint32_t disable_delay=1; SPI_HandleTypeDef handle; };
enum spi_status_e { SPI_OK, SPI_ERROR, SPI_TIMEOUT };
class SPIExClass {
    spi_t _spi;
    volatile spi_status_e spiFault=SPI_OK;
public:
    SPI_HandleTypeDef *getHandle() { return &_spi.handle; }
    spi_status_e getLastError() const { return spiFault; }
    spi_status_e transferChecked(uint8_t *,uint32_t);
};
static uint32_t ticks, calls, mask, lows, highs, sent, received, size, settleDelays;
static bool frozen, enabled, selected, systickWrap;
static uint8_t byte;
uint32_t micros() { assert(++calls<300000);return systickWrap?(ticks++%1000):frozen?ticks:ticks++; }
...
void digitalWrite(unsigned pin,unsigned value) {
    assert(pin==20 && mask==1);
    if(value) { assert(!enabled);highs++;selected=false; }
    else { lows++;selected=true; }
}
void delayMicroseconds(unsigned us) { assert(us==1);if(enabled)settleDelays++; }
...
void LL_SPI_Disable(SPI_TypeDef *) { if(received==size && size)assert(settleDelays);enabled=false; }
''' + bodies + '''
static void check(uint32_t flags, spi_status_e expected, bool freeze=false, bool wrap=false) {
    peripheral.SR=flags;calls=0;frozen=freeze;systickWrap=wrap;
    for(unsigned previous:{0u,1u}) {
        mask=previous;ticks=wrap?999:0xfffffff0;lows=highs=sent=received=settleDelays=0;
        ...
        assert(spi.getLastError()==expected);
        if(expected!=SPI_OK) {
            peripheral.SR=11; // A later healthy peripheral must not clear the fault.
            assert(spi.transferChecked(data,259)==expected);
            assert(lows==1 && highs==1 && mask==previous);
            peripheral.SR=flags;
        }
        if(expected==SPI_OK) {
...
int main() {
    check(11,SPI_OK);
    check(11,SPI_OK,false,true); // SysTick reloads while its millisecond ISR is masked.
    ...
    spi.getHandle()->State=HAL_SPI_STATE_RESET;
    assert(spi.transferChecked(&data,1)==SPI_ERROR);
    assert(lows==0 && highs==0); // No pin/peripheral access before initialization.
}
''')
