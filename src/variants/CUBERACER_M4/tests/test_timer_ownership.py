...
#define TIM1_BASE 1
#define TIM1 (&timer1)
static unsigned clockWrites,errors,granted,clockEnabled=1;
#define __HAL_RCC_TIM1_CLK_ENABLE() (++clockWrites)
#define __HAL_RCC_TIM1_CLK_DISABLE() (++clockWrites)
int cuberacerTimerClockAllowed(uintptr_t instance) { return instance==(uintptr_t)TIM1 && granted && clockEnabled; }
void Error_Handler(void) { errors++; }
''' + bodies + '''
int main(void) {
    enableTimerClock(TIM1);assert(clockWrites==0);assert(errors==1);
    granted=1;enableTimerClock(TIM1);disableTimerClock(TIM1);
    assert(clockWrites==0 && errors==1);
    enableTimerClock(&timer2);disableTimerClock(&timer2);assert(errors==3);
    clockEnabled=0;enableTimerClock(TIM1);assert(errors==4 && clockWrites==0);
}
''')
